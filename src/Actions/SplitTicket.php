...
/**
 * Extracts one or more messages from a ticket into a new ticket, keeping the two
 * linked, and emits {@see TicketSplit}.
 */
class SplitTicket
{
    ...
    public function handle(Ticket $source, array $messageIds, ?string $title = null, ?Model $actor = null): Ticket
    {
        return DB::transaction(function () use ($source, $messageIds, $title, $actor): Ticket {
            ...
            // Link the new ticket back to the source.
    ...
    protected function typeKey(Ticket $source): string
    {
        ...
        $type = $model::query()->withoutTenancy()->whereKey($source->ticket_type_id)->first();

        return is_string($type?->key) ? $type->key : 'support';
    }
}
