...
class CsatController
{
    public function store(StoreCsatRequest $request, Ticket $ticket): JsonResponse
    {
        if ($request->filled('token')) {
            ...
            $rating = Ticketing::submitCsatByToken($token, $request->integer('rating'), $request->input('comment'), $request->user());
        } else {
            $rating = Ticketing::submitCsat($ticket, $request->integer('rating'), $request->input('comment'), $request->user());
        }
